...
class AccountAuxiliaryReport(models.AbstractModel):
    ...
    def _get_lines(self, options, line_id=None):
        ...
        if 'book_id' in options.keys():
            ...
            dom = None
        data = self.movements_according_account(range_date=range_date, account_obje=accounts,
                                                analytical_filter=analytical, domain=dom)
    ...
    def get_analytical_order(self, opt_wizard):
        ...
        """
        list = [{'analytical': 'analytical_account', 'position': opt_wizard.analytical_account,
                 'data': opt_wizard.analytical_account_value, 'field_name': 'analytic_account_id'},
                {'analytical': 'associated', 'position': opt_wizard.associated, 'data': opt_wizard.associated_value,
                 'field_name': 'partner_id'},
                {'analytical': 'analytical_tag', 'position': opt_wizard.analytical_tag,
                 'data': opt_wizard.analytical_tag_value, 'field_name': 'analytic_tag_ids'}]
        pop = []
        for x in list:
            if int(x['position']) == 0:
                pop.append(x)
        ...
        if len(analytical_filter) == 0:
            ...
            for account in list_accounts:
                ...
                domain_or = [('date', '<', range_date[0]), ('account_id', '=', account.id),
                             ('parent_state', '=', "posted")]
                domain_total = [('date', '>=', range_date[0]), ('date', '<=', range_date[1]),
            ...
            for account in list_accounts:
                ...
                for filter in analytical_filter[0]['data']:
                    ...
                    domain_or = [('date', '<', range_date[0]), ('account_id', '=', account.id),
                                 (analytical_filter[0]['field_name'], '=', filter.id), ('parent_state', '=', "posted")]
                    domain_total = [('date', '>=', range_date[0]), ('date', '<=', range_date[1]),
                                    ('account_id', '=', account.id),
                                    (analytical_filter[0]['field_name'], '=', filter.id),
                                    ('parent_state', '=', "posted")]
                    if not domain is None:
                        domain_or.append(domain)
                        domain_total.append(domain)
                ...
                initial_balance = {'debit': list_bala[0]['debit'], 'credit': list_bala[0]['credit'],
                                   'balance': list_bala[0]['balance']}
                total_balance = {'debit': list_bala[1]['debit'], 'credit': list_bala[1]['credit'],
                                 'balance': list_bala[1]['balance']}

                if len(list_detail) > 0:
                    dic = {'number_filters': 1, 'account': account, 'detail': list_detail,
                           'initial_balance': initial_balance, 'total_balance': total_balance}
                    list_dic.append(dic)
            ...
            for account in list_accounts:
                ...
                for filter1 in analytical_filter[0]['data']:
                    ...
                    for filter2 in analytical_filter[1]['data']:
                        ...
                        domain_or = [('date', '<', range_date[0]), ('account_id', '=', account.id),
                                     (analytical_filter[0]['field_name'], '=', filter1.id),
                                     (analytical_filter[1]['field_name'], '=', filter2.id),
                                     ('parent_state', '=', "posted")]
                        domain_total = [('date', '>=', range_date[0]), ('date', '<=', range_date[1]),
                                        ('account_id', '=', account.id),
                                        (analytical_filter[0]['field_name'], '=', filter1.id),
                    ...
                    for filter2 in analytical_filter[1]['data']:
                        ...
                        for filter3 in analytical_filter[2]['data']:
                            ...
                            domain_or = [('date', '<', range_date[0]), ('account_id', '=', account.id),
                                         (analytical_filter[0]['field_name'], '=', filter1.id),
                                         (analytical_filter[1]['field_name'], '=', filter2.id),
                                         (analytical_filter[2]['field_name'], '=', filter3.id),
                                         ('parent_state', '=', "posted")]
                            domain_total = [('date', '>=', range_date[0]), ('date', '<=', range_date[1]),
                                            ('account_id', '=', account.id),
                                            (analytical_filter[0]['field_name'], '=', filter1.id),
    ...
    def calculate_balance_group(self, list_detail):
        ...
        return [{'debit': debit_balance, 'credit': credit_balance, 'balance': balance_balance},
                {'debit': debit_total, 'credit': credit_total, 'balance': balance_total}]
            ...
            if acc_all:
                for line in data:
                    if line['total_balance']['balance'] != 0:
                        ...
                        return_list.append({'id': 'initial_' + str(count),
                                            'level': 3,
                        ...
                        for move in line['move']:
                            dic_move = {'id': move.id,
                                        ...
                                        'columns': [{'name': str(move.journal_id.name)},
                                                    ...
                                                    {'name': (move.partner_id.name) if move.partner_id.name else ''},
                                                    ...
                                                    {'name': str(move.ref) if move.ref else '',
                                                     'class': 'whitespace_#print'},
                                                    {'name': str(move.name), 'class': 'whitespace_#print'}]}

                            return_list.append(dic_move)
                        return_list.append({'id': 'totals_' + str(count),
                                            'level': 3,
                                            'name': 'Total',
                                            'style': 'background: #b8b5ff; font-weight: bold;',
                                            'parent_id': dic_head['id'],
                ...
                for line in data:
                    if line['total_balance']['balance'] != 0:
                        ...
                        return_list.append({'id': 'initial_' + str(count),
                                            ...
                                            'name': _('Initial Balance Account'),
                                            'style': 'font-weight: bold;',
                                            'parent_id': dic_head['id'],
                                            'level': 3,
                                            'columns': [
                        ...
                        for filter_data in line['detail']:
                            count_fil += 1
                            dic_head_f = {
                                'id': filter_data['filter1'].name + str(filter_data['filter1'].id) + str(count),
                                'name': str(filter_data['filter1'].name if len(filter_data['filter1'].name) < 14 else
                                            filter_data['filter1'].name[:11] + "..."),
                                'title_hover': str(filter_data['filter1'].name),
                                'level': 4,
                            ...
                            return_list.append({'id': 'initial_' + str(count_fil),
                                                # 'class': 'o_account_reports_initial_balance',
                                                ...
                                                'columns': [
                                                    ...
                                                    {'name': self.format_value(
                                                        filter_data['initial_balance']['debit']),
                                                        'class': 'number'},
                                                    {'name': self.format_value(
                                                        filter_data['initial_balance']['credit']),
                                                        'class': 'number'},
                                                    {'name': self.format_value(
                                                        filter_data['initial_balance']['balance']),
                                                        'class': 'number'},
                                                    {'name': ''}]})
                    ...
                    if line['total_balance']['balance'] != 0:
                        ...
                        dic_head = {'id': "head" + str(line['account'].id),
                                    'name': str(line['account'].code) + " " + str(
                                        line['account'].name if len(line['account'].name) < 14 else line[
                                                                                                        'account'].name[
                                                                                                    :11] + "..."),
                        ...
                        for line_filter1 in line['detail']:
                            ...
                            dic_head_f_1 = {'id': str(line_filter1['filter1'].id) + str(count),
                                            'name': str(line_filter1['filter1'].name if len(
                                                line_filter1['filter1'].name) < 15 else line_filter1['filter1'].name[
                                                                                        :12] + "..."),
                            ...
                            for line_filter2 in line_filter1['detail']:
                                ...
                                dic_head_f_2 = {'id': str(line_filter2['filter2'].id) + str(count),
                                                'name': str(line_filter2['filter2'].name if len(
                                                    line_filter2['filter2'].name) < 15 else line_filter2[
                                                                                                'filter2'].name[
                                                                                            :12] + "..."),
                        ...
                        for line_filter1 in line['detail']:
                            ...
                            for line_filter2 in line_filter1['detail']:
                                ...
                                for line_filter3 in line_filter2['detail']:
                                    ...
                                    return_list.append(
                                        {'id': 'initial_' + str(count) + str(count_f_1) + str(count_f_2) + str(
                                            count_f_3),
                                         # 'class': 'o_account_reports_initial_balance',
                                         'name': 'Initial Balance',
                                         # 'style': 'background: #dddcf9;',
                                         ...
                                         'columns': [{'name': '', 'class': 'number'},
                                                     {'name': self.format_value(
                                                         line_filter3['initial_balance']['debit']),
                                                         'class': 'number'},
                                                     {'name': self.format_value(
                                                         line_filter3['initial_balance']['credit']),
                                                         'class': 'number'},
                                                     {'name': self.format_value(
                                                         line_filter3['initial_balance']['balance']),
                                                         'class': 'number'},
                                                     {'name': ''}], 'colspan': 4})
                                        ...
                                        {'id': 'totals_' + str(count),  # 'class': 'o_account_reports_initial_balance',
                                         ...
                                         'columns': [{'name': '', 'class': 'number'},
                                                     {'name': self.format_value(line_filter3['total_balance']['debit']),
                                                      'class': 'number'},
                                                     {'name': self.format_value(
                                                         line_filter3['total_balance']['credit']),
                                                         'class': 'number'},
                                                     {'name': self.format_value(
                                                         line_filter3['total_balance']['balance']),
                                                         'class': 'number'},
                                                     {'name': ''}],
                                         'colspan': 4})
    ...
    def get_xlsx(self, options, response=None):
        # return super(AccountAuxiliaryReport, self).get_xlsx(options, response)
    ...
    def print_xlsx_normal(self, options):
        ...
        for y in range(0, len(lines)):
            ...
            for x in range(2, len(lines[y]['columns']) + 2):
                ...
                cell_type_money, cell_value_money = self._get_cell_type_value_money(lines[y]['columns'][x - 2], x - 2,
                                                                                    len(lines[y]['columns']))
